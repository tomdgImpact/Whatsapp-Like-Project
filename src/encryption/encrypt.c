...
unsigned char *Hex_str_to_ascii_str(unsigned char *hex_str)
{
    ...
    size_t ascii_str_len;
    size_t j;

    ascii_str_len = 16;
    ascii_str = malloc(ascii_str_len * sizeof(unsigned char));

    j = 0;
    for (size_t i = 0; i < 32; i += 2)
    {
        ascii_str[j] = (hex_str[i] - '0') * 16 + (hex_str[i+1] - '0');
        j += 1;
    }

    return ascii_str; // to be freed
}
unsigned char *Encrypt(unsigned char *message, unsigned char *key_str)
{
    /*unsigned char str_message[] = "00112233445566778899AABBCCDDEEFF";
    for (int i = 0; i < 32; i++)
    {
        printf("%c", str_message[i]);
    }
    
    unsigned char *message = Hex_str_to_ascii_str(str_message);
    printf("%x\n", message);*/
    ...
    for (int i = 0; i < lenOfPaddedMessage; i+=16)
        AES_encryption(paddedMessage + i, key_str);
